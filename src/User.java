 ...
 * Description: This class to make sure the program will only input positive numbers.
 */
...
public class User {
	...
	public static double inputPositiveDouble(String prompt) {
		...
		while(isPositive) {
			...
			value = inputDouble();
			if(value>0) {
				isPositive = false; // break out of loop
			}
			else {
				System.out.print("Invalid input. Enter a positive: ");
			}
		 // only returns 0.0 so class will compile, replace with user provided value
		}
		return value;
	}
}
